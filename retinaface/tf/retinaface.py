...
class RetinaFace:
  def __init__(self, modelPath, network='net3', nms=0.4, nocrop=False, decay4 = 0.5, vote=False):
    self.ctx_id = 0
    ...
    with tf.Graph().as_default():
      ...
      with gfile.FastGFile(modelPath, 'rb') as f:
        ...
        tf.import_graph_def(graphDef, input_map=None, name='')
        model = tf.get_default_graph()
        outNames = ["face_rpn_cls_prob_reshape_stride32:0", "face_rpn_bbox_pred_stride32:0", "face_rpn_landmark_pred_stride32:0", "face_rpn_cls_prob_reshape_stride16:0", "face_rpn_bbox_pred_stride16:0", "face_rpn_landmark_pred_stride16:0", "face_rpn_cls_prob_reshape_stride8:0", "face_rpn_bbox_pred_stride8:0", "face_rpn_landmark_pred_stride8:0"]
        self.outTensors = []
        self.inputsTensor = model.get_tensor_by_name("data:0")
        for name in outNames:
          tensor = model.get_tensor_by_name(name)
          self.outTensors.append(tensor)
  ...
  def detect(self, img, threshold=0.5, scales=[1.0], do_flip=False):
    ...
    for im_scale in scales:
      for flip in flips:
        ...
        im_tensor = np.zeros((1, im.shape[0], im.shape[1], 3))
        for i in range(3):
            im_tensor[0, :, :, i] = (im[:, :, 2 - i]/self.pixel_scale - self.pixel_means[2 - i])/self.pixel_stds[2-i]
        if self.debug:
          timeb = datetime.datetime.now()
        ...
        feedDict = {self.inputsTensor : im_tensor}
        net_out = self.sess.run(self.outTensors, feed_dict=feedDict)
