...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape, path_imgrec,
                 ...
                 color_jittering = 0, images_filter = 0,
            ...
            if header.flag>0:
                ...
                for identity in self.seq_identities[k]:
                    ...
                    if count < images_filter:
                        continue
                    self.id2ranges[k][identity] = (a,b) #identity range
                    imgidx += range(a, b)
                self.imgidxes.append(imgidx)
    ...
    def reset(self):
        ...
        for k in range(2):
            if self.seqs[k] is None and self.imgrecs[k] is not None:
                self.imgrecs[k].reset()

    def num_samples(self, recIdx):
        return len(self.seqs[recIdx])

    def next_sample(self, recIdx):
        """Helper function for reading in next sample."""
        #set total batch size, for example, 1800, and maximum size for each people, for example 45
        if self.seqs[recIdx] is not None:
            if self.curs[recIdx] >= len(self.seqs[recIdx]):
                raise StopIteration
            idx = self.seqs[recIdx][self.curs[recIdx]]
            self.curs[recIdx] += 1
            s = self.imgrecs[recIdx].read_idx(idx)
            ...
            for cc in range(self.ctx_num):
                for kk in range(2):
                    ...
                    while (i < batch_size//self.ctx_num//2):
                        ...
                        for datum in data:
                            assert i < batch_size//self.ctx_num//2, 'Batch size must be multiples of augmenter output length'
                            #print(datum.shape)
