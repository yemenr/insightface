...
def parse_args():
  ...
  parser.add_argument('--auxloss', default=default.auxloss, help='auxloss config')
  args, rest = parser.parse_known_args()
  generate_config(args.network, args.dataset, args.loss, args.auxloss)
...
def get_symbol(args):
  ...
  if config.loss_name=='softmax': #softmax
    _weight = mx.symbol.Variable("fc7_weight", shape=(args.id_num_classes, config.emb_size), 
        lr_mult=config.fc7_lr_mult, wd_mult=config.fc7_wd_mult, init=mx.init.Normal(0.01))
    if config.fc7_no_bias:
      fc7 = mx.sym.FullyConnected(data=embedding, weight = _weight, no_bias = True, num_hidden=args.id_num_classes, name='fc7')
    else:
      _bias = mx.symbol.Variable('fc7_bias', lr_mult=2.0, wd_mult=0.0)
      fc7 = mx.sym.FullyConnected(data=embedding, weight = _weight, bias = _bias, num_hidden=args.id_num_classes, name='fc7')
    ...
    fc7 = mx.sym.FullyConnected(data=nembedding, weight = _weight, no_bias = True, num_hidden=args.id_num_classes, name='fc7')
    if config.loss_m1!=1.0 or config.loss_m2!=0.0 or config.loss_m3!=0.0:
      if config.loss_m1==1.0 and config.loss_m2==0.0:
        s_m = s*config.loss_m3
        gt_one_hot = mx.sym.one_hot(gt_label, depth = args.id_num_classes, on_value = s_m, off_value = 0.0)
        fc7 = fc7-gt_one_hot
      else:
        zy = mx.sym.pick(fc7, gt_label, axis=1)
        cos_t = zy/s
        t = mx.sym.arccos(cos_t)
        if config.loss_m1!=1.0:  # m1 * theta
          t = t*config.loss_m1
        if config.loss_m2>0.0:  # theta + m2
          t = t+config.loss_m2
        body = mx.sym.cos(t)
        if config.loss_m3>0.0:  # cos(theta) - m3
          body = body - config.loss_m3
        new_zy = body*s
        diff = new_zy - zy
        diff = mx.sym.expand_dims(diff, 1)
        gt_one_hot = mx.sym.one_hot(gt_label, depth = args.id_num_classes, on_value = 1.0, off_value = 0.0)
  ...
  if is_softmax:
    ...
    #  _label = mx.sym.one_hot(gt_label, depth = args.id_num_classes, on_value = -1.0, off_value = 0.0)
    #  body = body*_label
    ...
    _label = mx.sym.one_hot(idLabels, depth = args.id_num_classes, on_value = -1.0, off_value = 0.0)
    body = body*_label
    ...
    if (config.dataset == 'multidata') and (args.lsr == True):
      sequence_loss_ = -mx.symbol.sum(mx.symbol.mean(mx.symbol.log_softmax(seqLogits),axis=1))/args.per_batch_size # warning
      sequence_loss = mx.symbol.MakeLoss(name='sequence_loss', data=sequence_loss_, normalization='valid',grad_scale=args.seq_loss_factor)  
      out_list.append(sequence_loss)
    
    # auxiliary loss
    ## center loss
    if args.auxloss == 'center': 
    ...
    ## git loss
      ...
      nembedding = mx.symbol.L2Normalization(embedding, mode='instance')  # do norm
      git_loss_ = mx.symbol.Custom(data=nembedding, label=gt_label, name='git_loss_', op_type='gitloss', id_num_class=args.id_num_classes, seq_num_class=args.seq_num_classes, git_params=(config.center_alpha, config.git_alpha, config.git_beta, config.git_p), scale=args.aux_loss_factor, batchsize=args.per_batch_size)
...
def train_net(args):
    ...
    prefix = os.path.join(args.models_root, curTime, '%s-%s-%s'%(args.network, args.loss, args.auxloss), 'model')
    ...
    if config.dataset != 'multidata':
      ...
      path_imgrec = None      
      args.id_num_classes = config.num_classes
      args.seq_num_classes = 0
      print('id_num_classes', args.id_num_classes)
      path_imgrec = os.path.join(data_dir, "train.rec")
    else:
      data_dir = config.dataset_path[0]
      seq_data_dir = config.dataset_path[1]
      path_imgrec = None      
      seq_path_imgrec = None
      args.id_num_classes = config.num_classes[0]
      args.seq_num_classes = config.num_classes[1]
      print('id_num_classes: %d, seq_num_classes: %d' % (config.num_classes[0], config.num_classes[1]))
      path_imgrec = os.path.join(data_dir, "train.rec")
      seq_path_imgrec = os.path.join(seq_data_dir, "train.rec")
